...
SafetyPredictor encapsulates the full ML lifecycle:
  - Feature engineering (spatial, tags, grid cell, temporal, night flag)
...
Dataset schema (hervoice_ml_training_data3.csv):
  - device_uuid       → dropped (not a predictive feature)
  - lat/lng           → renamed to latitude/longitude internally
  - grid_cell_id      → label-encoded as grid_encoded
  - safety_rating     → target column
  - time_context      → hour of day (0-23); used for cyclic encoding + is_night
  - created_at        → parsed for day_of_week, month, hour
  - tags              → multi-label binarized
  Note: zone_label and is_night are absent in this dataset;
        is_night is derived from time_context automatically.
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
CSV_PATH        = "hervoice_ml_training_data3.csv"
...
def build_features(
    ...
    Columns used:
      • lat/lng          → latitude/longitude (renamed in train())
      • tags             → multi-label binarized
      • grid_cell_id     → label-encoded
      • created_at       → day_of_week, month, hour
      • time_context     → overrides hour; source for hour_sin/cos + is_night
      Note: zone_label absent in this dataset — zone_encoded dropped.
    ...
    # ── 5. time_context — override parsed hour when available ─────────────────
    ...
    # ── 5b. Cyclic hour encoding — makes hour 23 and hour 0 numerically adjacent
    temporal_df["hour_sin"] = np.sin(2 * np.pi * temporal_df["hour"] / 24)
    temporal_df["hour_cos"] = np.cos(2 * np.pi * temporal_df["hour"] / 24)

    # ── 6. is_night flag — always derived from time_context/hour ──────────────
    is_night = _compute_is_night(temporal_df["hour"])

    # ── Combine ────────────────────────────────────────────────────────────────
    X = pd.concat(
        [spatial_df, tag_df, grid_encoded, temporal_df, is_night],
        axis=1,
    )
    return X, mlb, le
...
def train(csv_path: str = CSV_PATH) -> dict:
    ...
    df.drop(columns=[c for c in ["device_uuid"] if c in df.columns], inplace=True)
...
def predict_safety(
    ...
    time_context : int   — hour of day (0-23)
...
class SafetyPredictor:
    ...
    def predict_batch(self, records: list[dict]) -> list[float]:
        ...
        Each record must be a dict with keys:
            latitude, longitude, tags, grid_cell_id, created_at, time_context
        """
        if not self._is_ready:
